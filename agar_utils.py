...
def set_K(aj):
    global K, output_dir, model_dir, figures_dir, weights_dir, meas_dir 
    ...
    if K is not None:
        ...
        print("Value of k has not been set yet")
    ...
    return K, output_dir, model_dir, figures_dir, weights_dir, meas_dir 
...
def mm_predict(m_obs, gain, da, la, xob):
    ...
    K = gain
    set_K(gain)
    a = np.load(f'{weights_dir}/weights_{da}_lambda_{la}.npy', allow_pickle=True) 

    num_observers = len(m_obs)
    tt = xob[:, 4]
