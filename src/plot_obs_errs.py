...
gt_cool_1 = {k.replace('meas_cool_1_', '').replace('_0.14', ''): v for k, v in sorted(a.items()) if k.startswith('meas_cool_1') and k.endswith('0.14')}
...
gt_cool_1 = dict(sorted(gt_cool_1.items(), key=lambda item: int(item[0])))
length = len(gt_cool_1["8"])

times = np.array([np.linspace(0, 1, num=length)]*3)*1800
vals = [ v for v in gt_cool_1.values()]
legend_labels = [fr"$n_{{\mathrm{{obs}}}}={k}$" for k in gt_cool_1.keys()]
...
#     filename=f"{output_folder}/l2_cooling_2_rsc.png",
#     size=(6, 5),
...
pp.plot_generic(
    ...
    xlabel=r"$t \, (s)$",
    legend_labels=legend_labels,
    filename=f"{output_folder}/gt_cooling_1.png",
    size=(6, 5),
    colors=["#1f77b4", "#2ca02c", "#d62728"]  # Different shades of blue, green, and red
    )
