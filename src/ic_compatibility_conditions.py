...
opt_pinns_obs_ic = uu.ic_obs(x_pinns_obs_ic, b2=b2_opt.round(4), b3=b2_opt.round(4))

x = [x_pinns_obs_ic, x_measurements_ic]
y = [opt_pinns_obs_ic, measurements_ic]
title = "Comparison at t=0" if rescale else r"Comparison at $\tau=0$"
xlabel, _, ylabel = uu.get_scaled_labels(rescale)
legend_labels = ["Obs PINNs", "Measurements"]
fname = f"{src_dir}/data/vessel/t0_{set[0]}_{set[1]}.png"
sys_colors, _ = uu.get_sys_mm_colors(conf)
obs_colors = uu.get_obs_colors(conf)
sys_linestyle, obs_linestyle = uu.get_sys_mm_linestyle(conf)
colors = [obs_colors[1], sys_colors]
