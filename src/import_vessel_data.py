...
for date in dates:
    ...
    # print(df)
    print(df['y1'][0], df['y2'][0], df['y3'][0])
    df1 = scale_df(df)
    save_to_pickle(df1, f"{src_dir}/data/vessel/{name}.pkl")
    ...
    ax.set_title(f"{date}", fontweight='bold')
    ...
    plt.savefig(f"{src_dir}/data/vessel/{name}.png", dpi=120)
    plt.show()
    plt.close()
