...
output_dir = f"{script_directory}/multiple_observer_smaller_range"
...
def create_observer(alpha, h):
    ...
    def ic_obs(x):
        ...
        e = (fl + k * (t2 - t1))/(b1 * tf.cos(b1) + k * tf.sin(b1)) * tf.sin(b1*x[:, 0:1]) + t1
        return e
...
h_unk = [2.73e+00, 5.46e+00, 10.92e+00]
k_unk = [1.945e-01, 3.89e-01, 7.78e-01]
...
for kk in k_unk:
    for hh in h_unk:
        modelu = create_observer(kk, hh)
        modelu = restore_model(modelu, f"obs_{kk}_{hh}")
        multi_obs[(kk, hh)] = modelu
